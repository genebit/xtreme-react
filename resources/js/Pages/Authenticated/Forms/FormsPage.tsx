...
<!-- Centered Confirm / Danger Modal -->
<div class="modal fade" id="confirmModal" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered modal-sm">
    <div class="modal-content">
      <div class="modal-body text-center px-4 py-4">
        <div class="text-danger fs-1 mb-2"><i class="ri-error-warning-line"></i></div>
        <h6 class="fw-bold">Delete this item?</h6>
...
const CODE_SHEET = `// Sheet.tsx — import and use anywhere
import Sheet from "@/Components/Sheet";
import { useState } from "react";
...
function SectionCard({
  ...
  return (
    <Card className="tw-border-0 tw-shadow-sm tw-rounded-2xl tw-mb-6">
      <Card.Body className="tw-p-6">
        <div className="tw-flex tw-items-start tw-justify-between tw-mb-5 tw-pb-4 tw-border-b tw-border-slate-100">
          <div>
            <h2 className="tw-text-base tw-font-bold tw-text-slate-800">{title}</h2>
            {description && (
              <p className="tw-text-xs tw-text-slate-400 tw-mt-0.5">{description}</p>
            )}
          </div>
          {code && (
            <button
              ...
              className="tw-flex tw-items-center tw-gap-1.5 tw-shrink-0 tw-ml-3 tw-text-xs tw-font-medium tw-text-slate-500 hover:tw-text-slate-700 tw-bg-slate-100 hover:tw-bg-slate-200 tw-transition-colors tw-px-3 tw-py-1.5 tw-rounded-md tw-whitespace-nowrap"
            >
            ...
            <div className="tw-relative">
              <div className="tw-absolute tw-top-3 tw-right-3 tw-z-10">
                <button
                  type="button"
                  onClick={handleCopy}
                  className="tw-flex tw-items-center tw-gap-1 tw-text-xs tw-text-slate-400 hover:tw-text-white tw-bg-slate-700/70 hover:tw-bg-slate-600 tw-transition-colors tw-px-2.5 tw-py-1.5 tw-rounded"
                >
                  {copied ? <RiCheckLine size={12} /> : <RiFileCopyLine size={12} />}
                  {copied ? "Copied!" : "Copy"}
                </button>
              </div>
              <pre className="tw-bg-[#1e293b] tw-text-slate-300 tw-rounded-xl tw-p-4 tw-pt-10 tw-text-xs tw-leading-relaxed tw-overflow-x-auto tw-font-mono tw-m-0 tw-whitespace-pre">
                <code>{code}</code>
              </pre>
            </div>
          </div>
...
export default function FormsPage() {
  ...
  return (
    <AdminLayout headTitle="Form Layouts">
      <style>{`
        @keyframes rsMenuIn {
      ...
      <PageHero
        title="Form Layouts"
        subtitle="Bootstrap 5 form components — inputs, selects, validation, groups, modals, and more."
      />
      ...
      >
        ...
        <div className="tw-border-t tw-border-slate-100 tw-pt-6">
          <p className="tw-text-xs tw-font-semibold tw-text-slate-400 tw-uppercase tw-tracking-wide tw-mb-3">
